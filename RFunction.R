...
rFunction <- function(data, selName=NULL, startVar=NULL, endVar=NULL, trackVar="individual.local.identifier")
{
  Sys.setenv(tz="UTC") 
  ...
  } else
  {
    ...
    data_filter <- foreach(datai = data.split) %do% {
      ...
      {
        if (any(selT@data$individual.local.identifier==id)) time1 <- as.POSIXct(selT@data[which(selT@data[,trackVar]==id),startVar]) else time1 <- NA
      }
      ...
      {
        if (any(selT@data$individual.local.identifier==id)) timeN <- as.POSIXct(selT@data[which(selT@data[,trackVar]==id),endVar]) else timeN <- NA  
      }
      
      if (is.na(time1) | is.na(timeN)) dataif <- NULL else
      {
        if (time1>timeN)
        {
          time10 <- time1
